...
namespace BankTransaction.Web.Controllers
{
    ...
    public class PersonController : Controller
    {
        ...
        [HttpGet]
        [Authorize(Roles = "Admin")]
        public async Task<IActionResult> GetAllPersons([FromQuery] PersonSearchModel personSearch = null,
            PageQueryParameters pageQueryParameters = null)
        {
            var filter = mapper.Map<PersonFilterModel>(personSearch);
            var allPersons =
                await personService.GetAllPersons(pageQueryParameters.PageNumber, pageQueryParameters.PageSize, filter);
        ...
        public async Task<IActionResult> UpdatePerson(int id)
        {
            ...
            if (currentPerson == null) return NotFound($"Person with id {id} not find");
        ...
        {
            if (personModel == null) return BadRequest("Object of type person is null");
        ...
        {
            var currentPerson = id == 0
                ? await personService.GetPersonById(HttpContext.User)
                : await personService.GetPersonById(id);

            if (currentPerson == null) return NotFound("Person  not find");
